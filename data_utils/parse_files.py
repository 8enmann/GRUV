...
def convert_np_audio_to_sample_blocks(song_np, block_size):
  '''Chop up song_np into block_size equal sized blocks, padding with zeros if necessary.
  '''
  total_samples = song_np.shape[0]
  even_samples = total_samples / block_size
  remainder = total_samples % block_size
  x = np.split(song_np[:even_samples * block_size], even_samples)
  if remainder:
    x += [np.concatenate((song_np[-remainder:], np.zeros(block_size - remainder)))]
  return x
...
def fft_blocks_to_time_blocks(blocks_ft_domain):
  ...
  for block in blocks_ft_domain:
    real_chunk, imag_chunk = np.split(block, 2)
